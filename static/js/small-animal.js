...
if (!password) {
    ...
    document.getElementById('submit-password-btn').addEventListener('click', (_) => {
...
function runGame() {
    ...
    nextQuestion()
    document.getElementById('next-question-btn').addEventListener('click', (_) => {
        nextQuestion()
    })
}

function nextQuestion() {
    const currentQuestion = questions.splice(Math.floor(Math.random() * questions.length), 1)[0]
    if (!currentQuestion) {
